...
def get_critical_cves(page=1):
    """Récupère les CVEs critiques"""
    url = f"https://app.opencve.io/api/cve?vendor={vendor}&"
    params = {
        "cvss": "critical",
        "page": page
    }
    ...
    try:
        ...
        print(f"Erreur lors de la récupération des CVEs critiques: {e}")
        return []
...
def format_cve_details(cve_data):
    """Formate les détails complets d'une CVE critique"""
    if not cve_data:
...
def send_critical_cve_report(recipient, page=1):
    """Envoie le rapport des CVEs critiques par email"""
    cves = get_critical_cves(page)
    
    if not cves:
        print("Aucune CVE critique trouvée")
        return False

    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    email_content = f"Rapport des CVEs critiques (CVSS = Critical)\n"
    email_content += f"Page: {page} | Généré le: {now}\n\n"
    email_content += f"Nombre de CVEs critiques trouvées: {len(cves)}\n\n"
    ...
    msg = EmailMessage()
    msg['Subject'] = f'Rapport CVEs Critiques (Page {page})'
...
if __name__ == "__main__":
    print("=== Envoi des CVEs critiques par email ===")
    page = input("Entrez le numéro de page (1 par défaut): ").strip() or "1"
    vendor = input("Veuillez entrez le vendeurs d'on vous voulez afficher les CVE: ")
    recipient = input("Entrez l'email destinataire: ").strip()
    
    send_critical_cve_report(recipient, page)